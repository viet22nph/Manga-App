...
public class GetReplyCommentQueryHandler : IQueryHandler<GetRepliesQuery, Pagination<ReplyResponse>>
{
    private readonly IUnitOfWork _unitOfWork;
    public GetReplyCommentQueryHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }


    public async Task<Result<Pagination<ReplyResponse>>> Handle(GetRepliesQuery request, CancellationToken cancellationToken)
    {

        var commentSelect =
             _unitOfWork
            .CommentRepository
            .FindAll(x => x.Type == request.Type && x.CommentTypeId == request.TargetId && x.ParentId == request.ParentId)
        ...
        return Pagination<ReplyResponse>.Create(commentResponse, commentTotal, request.PageIndex, request.PageIndex);
    }
}
