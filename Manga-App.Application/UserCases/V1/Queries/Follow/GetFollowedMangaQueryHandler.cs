...
namespace MangaApp.Application.UserCases.V1.Queries.Follow;

public class GetFollowedMangaQueryHandler : IQueryHandler<GetFollowedMangaQuery, Pagination<MangaResponse>>
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly IAwsS3Service _awsS3Service;
    public GetFollowedMangaQueryHandler(IUnitOfWork unitOfWork, IAwsS3Service awsS3Service)
    {
        _unitOfWork = unitOfWork;
        _awsS3Service = awsS3Service;
    }
    public async Task<Result<Pagination<MangaResponse>>> Handle(GetFollowedMangaQuery request, CancellationToken cancellationToken)
    {
        var mangaQueryable = _unitOfWork
            .FollowRepository.FindAll(x => x.UserId == request.UserId)
        ...
        var listMangaResponse = await mangaQueryable
            .Skip((request.PageIndex - 1) * request.PageSize)
            .Take(request.PageSize)
            .ToListAsync(cancellationToken);
        var totalCountManga = await mangaQueryable.CountAsync(cancellationToken);   
